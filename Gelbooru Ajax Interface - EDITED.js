// ==UserScript==
// @name           Gelbooru Ajax Interface - EDITED
...
dai_css.appendChild(d.createTextNode("body { margin: 4px; } a { text-decoration: none; color: #0000EE; } img { object-fit: contain; border: 0px; } .thumb { border: 1px solid lightgrey /* WhiteSmoke */; min-width: 190px; min-height: 170px; max-width: 190px; max-height: 170px; margin: 3px /* -1px 0px 0px -1px */; padding: 1px; display: flex; justify-content: center; align-items: center; font-size: small; overflow: hidden; } .yellow { background-color: LightYellow; } .red { background-color: MistyRose; } .trans { border: 1px solid Black; background-color: LightYellow; position: absolute; } *:not(body) { transition: all 0.5s ease-in-out 0s; }                 /******* my addon *****//* .thumb > a { height: 100%; height: 100%; min-width: 190px; }*/ .thumb > a { height: auto; height: 100%; min-width: 190px; }"));
...
if(cacheTags.length == 0) {
    popularTags = "touhou kantai_collection magmallow ponchii sound greatm8 loli video sort:popular animated assertive_female bestiality".split(" ").reverse();
...
aTableBar.setAttribute("style", "border-collapse: collapse; border: 1px solid Black; width: 90%; padding: 0px");
        ...
        setTimeout(function() { // transitions are stupid
            ...
            if(aSD.checked) {
                for(var n = 0, deleted = null; deleted = deletedlist.snapshotItem(n++); n) {
                    ...
                    deleted.style.setProperty("min-width", "180px", "");
                    deleted.style.setProperty("max-width", "200px", "");
                }
            } else {
                cacheHide.length = 0;
                for(var n = 0, deleted = null; deleted = deletedlist.snapshotItem(n++); n) {
...
overlay.setAttribute("style", "display: none; position: fixed; top: 0px; left: 0px; width: 85%; height: 100%; background-color: Black; opacity: 0.8;");
innerDisplay = d.createElement("DIV");
innerDisplay.setAttribute("id", "innerDisplay");
innerDisplay.setAttribute("style", "background-color: White; display: inline-table; padding: 20px; min-width: 220px; min-height: 220px; border-radius: 10px; margin: auto;");
...
prevImage.setAttribute("style", "display: flex; justify-content: center; align-items: center; position: fixed; top: 0px; left: 0px; background-image: linear-gradient(to right, Black, Transparent); color: White; font-size: large; width: 20px; height: 100%; opacity: 0.3;");
prevImage.setAttribute("class", "fa fa-chevron-left");

nextImage = d.createElement("DIV");
nextImage.setAttribute("id", "nextImage");
nextImage.setAttribute("style", "display: flex; justify-content: center; align-items: center; position: fixed; top: 0px; right: 0px; background-image: linear-gradient(to left, Black, Transparent); color: White; font-size: large; width: 20px; height: 100%; opacity: 0.3;");
...
prevImage.addEventListener("mouseout", function(e) {
    prevImage.style.setProperty("opacity", "0.3", "");
}, false);
...
nextImage.addEventListener("mouseout", function(e) {
    nextImage.style.setProperty("opacity", "0.3", "");
}, false);
...
function showContent(xmldoc) {
    ...
    aReply.textContent = "Nothing found!!";
