...
// match          *://chan.sankakucomplex.com/#*
        ...
        {
        name    :   "Sankaku",                                 // Site name
        site    :   "chan\.sankakucomplex\.com",                               // Regular expression check on current url
        post    :   "/posts.xml",                               // Relative url to xml post API
        note    :   "/notes.xml",                               // Relative url to xml note API
        list    :   "/posts/",                                  // Relative url to post listing
        page    :   "/posts/",                                  // Relative url to post page
...
var tags = "", images = 50, page = 1, rating = "e", sampleRate = 1, blacklist = "spoilers \nguro \nscat \nfuta_on_futa \nfuta_on_male \nmale_sucking_futa \nfuta_only \nmale_focus \nmontage \nsolo_futa \ncompilation \ngreyscale \nmonochrome \ncosplay \nscreencap ";
...
dai_css.appendChild(d.createTextNode("body { margin: 4px; } a { text-decoration: none; color: #0000EE; } img { border: 0px; } .thumb { border: 1px solid WhiteSmoke; zoom: 1.3; min-width: 160px; min-height: 100px; max-width: 160px; max-height: 100px; margin: -1px 0px 0px -1px; padding: 1px; display: flex; justify-content: center; align-items: center; font-size: small; overflow: hidden; } .yellow { background-color: LightYellow; } .red { background-color: MistyRose; } .trans { border: 1px solid Black; background-color: LightYellow; position: absolute; } *:not(body) { transition: all 1s ease-in-out 0s; }"));
...
if(cacheTags.length == 0) {
    popularTags = "touhou kantai_collection magmallow ponchii sound greatm8 loli video sort:popular animated rating:explicit bestiality".split(" ").reverse();
        ...
        setTimeout(function() { // transitions are stupid
            ...
            if(aSD.checked) {
                for(var n = 0, deleted = null; deleted = deletedlist.snapshotItem(n++); n) {
                    ...
                    deleted.style.setProperty("min-width", "160px", "");
                    deleted.style.setProperty("max-width", "160px", "");
                }
            } else {
                cacheHide.length = 0;
                for(var n = 0, deleted = null; deleted = deletedlist.snapshotItem(n++); n) {
...
prevImage.setAttribute("style", "display: flex; justify-content: center; align-items: center; position: fixed; top: 0px; left: 0px; background-image: linear-gradient(to right, Black, Transparent); color: White; font-size: xx-large; width: 10px; height: 100%; opacity: 0;");
prevImage.setAttribute("class", "fa fa-chevron-left");

nextImage = d.createElement("DIV");
nextImage.setAttribute("id", "nextImage");
nextImage.setAttribute("style", "display: flex; justify-content: center; align-items: center; position: fixed; top: 0px; right: 0px; background-image: linear-gradient(to left, Black, Transparent); color: White; font-size: xx-large; width: 10px; height: 100%; opacity: 0;");
